...
class ThreeDimensionalDataset(Dataset):
    """
            ...
            one_hot: bool, default = False
                Should be set equal to True if the GT masks are available in a one-hot encoded fashion
                This is not applicable in a 3D setting since any pixel can only be assigned to one 3D GT object
            ...
            anisotropy_factor: float
                Should be set equal to the ratio of the pixel sizes along the z dimension and the x dimension
                In case, the raw image is acquired such that the image is down-sampled along the z-diemnsion,
                then 'anisotropy_factor' should be greater than equal to 1.0

            sliced_mode: bool
                In case the 3D images should be interpreted in a 'sliced fashion' (i.e. a 2D model should be used for training on individual slices
                And the predictions from these slices considered independently are combined during inference to generate a 3D instance mask), then
    ...
    def __getitem__(self, index):
        ...
        if self.type == 'test' and self.sliced_mode:
            ...
            image = image[::self.uniform_ds_factor, ::self.uniform_ds_factor, ::self.uniform_ds_factor]
