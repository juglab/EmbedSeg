...
from EmbedSeg.utils.utils import AverageMeter, Cluster, Logger, Visualizer, prepare_embedding_for_train_image
...
def begin_training(train_dataset_dict, val_dataset_dict, model_dict, loss_dict, configs):
    ...
    # set criterion
    criterion = get_loss(configs['grid_y'], configs['grid_x'], configs['pixel_y'], configs['pixel_x'],
                         configs['one_hot'],
                         loss_dict['lossOpts'])
    ...
    # clustering
    cluster = Cluster(configs['grid_y'], configs['grid_x'], configs['pixel_y'], configs['pixel_x'], configs['one_hot'])
